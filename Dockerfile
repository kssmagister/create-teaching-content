...
# Typst + gebuendelte Fonts + Python-Web-Schicht. Bewusst OHNE Docling
# (das waere ein GB-schweres ML-Image -> separater Schritt).
FROM python:3.12-slim

ARG TYPST_VERSION=0.15.0

# Typst-CLI + DejaVu als Symbol-/Fallback-Font (fuer ✓ / ⚠ etc.)
RUN apt-get update \
 && apt-get install -y --no-install-recommends curl xz-utils ca-certificates fonts-dejavu-core \
